...
class _UnpaidSalesByProductReportPageState
  ...
  Future<void> _exportPdf() async {
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        header: (context) => pw.Column(
          ...
          children: [
            pw.Text('Unpaid Sales by Product Report',
        ...
        build: (context) => [
          pw.TableHelper.fromTextArray(
            ...
            headers: ['S/N', 'Product', 'Category', 'Last Sale', 'Total', 'Paid', 'Balance', 'Profit'],
            data: [
              ...items.map((i) => [
                    '${i.sn}', i.product, i.category, i.lastSale,
                    _fmt(i.total), _fmt(i.paid), _fmt(i.balance), _fmt(i.profit),
                  ]),
              ['', 'TOTAL', '', '', _fmt(_totalTotal), _fmt(_totalPaid), _fmt(_totalBalance), _fmt(_totalProfit)],
            ],
          ),
        ],
    ...
    final file = File('${dir.path}/UnpaidSalesByProductReport_$_exportDateTime.pdf');
  ...
  Future<void> _exportExcel() async {
    ...
    excel.rename(excel.getDefaultSheet()!, 'Unpaid Sales by Product Report');

    final sheet = excel['Unpaid Sales by Product Report'];

    final headers = ['S/N', 'Product', 'Category', 'Last Sale', 'Total', 'Paid', 'Balance', 'Profit'];
    ...
    for (final item in items) {
      sheet.appendRow([
        ...
        xls.TextCellValue(item.lastSale),
    ...
    final dir = await getApplicationDocumentsDirectory();
    final file = File('${dir.path}/UnpaidSalesByProductReport_$_exportDateTime.xlsx');
  ...
  Widget _buildTable(List<_UnpaidProductData> items) {
    return Container(
      margin:
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _headerCell('Category', 90),
          _headerCell('Last Sale', 85),
  ...
  Widget _buildTableRow(_UnpaidProductData item, int index) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _nameCell(item.product, 100),
          _dataCell(item.category, 90),
          _dataCell(item.lastSale, 85),
          _dataCell(_fmt(item.total), 80),
          _dataCell(_fmt(item.paid), 70),
          _dataCell(_fmt(item.balance), 75),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _totalCell(_fmt(_totalPaid), 70),
          _totalCell(_fmt(_totalBalance), 75),
