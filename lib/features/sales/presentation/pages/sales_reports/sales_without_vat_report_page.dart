...
class _SalesWithoutVatReportPageState extends ConsumerState<SalesWithoutVatReportPage> {
  ...
  Future<void> _loadItems({String? from, String? to}) async {
    try {
      ...
      final data = await repo.fetchReportSalesWithoutVat(from: from, to: to);
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _exportPdf() async {
      ...
      pw.MultiPage(
        ...
        build: (context) => [
          pw.TableHelper.fromTextArray(
            ...
            headers: ['S/N', 'Date', 'Type', 'Total', 'Paid', 'Balance'],
            data: [
              ...items.map((i) => [
                    '${i.sn}', i.date, i.type,
                    _fmt(i.total), _fmt(i.paid), _fmt(i.balance),
                  ]),
              ['', '', 'TOTAL', _fmt(_totalTotal), _fmt(_totalPaid), _fmt(_totalBalance)],
            ],
          ),
        ],
    ...
    final file = File('${dir.path}/SalesWithoutVatReport_$_exportDateTime.pdf');
  ...
  Future<void> _exportExcel() async {
    ...
    excel.rename(excel.getDefaultSheet()!, 'Sales without VAT Report');

    final sheet = excel['Sales without VAT Report'];

    final headers = ['S/N', 'Date', 'Type', 'Total', 'Paid', 'Balance'];
    ...
    final dir = await getApplicationDocumentsDirectory();
    final file = File('${dir.path}/SalesWithoutVatReport_$_exportDateTime.xlsx');
  ...
  Widget _buildTable(List<_NonVatSaleData> items) {
    return Container(
      margin:
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _headerCell('Type', 120),
          _headerCell('Total', 100),
          _headerCell('Paid', 100),
  ...
  Widget _buildTableRow(_NonVatSaleData item, int index) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _dataCell(item.date, 100),
          _nameCell(item.type, 120),
          _dataCell(_fmt(item.total), 100),
          _dataCell(_fmt(item.paid), 100),
          _dataCell(_fmt(item.balance), 100),
        ],
      ),
    );
  }
    ...
    return Container(
      ...
      child: Row(
        children: [
          _totalCell('', 40),
          _totalCell('', 100),
          _totalLabelCell('TOTAL', 120),
          _totalCell(_fmt(_totalTotal), 100),
          _totalCell(_fmt(_totalPaid), 100),
          _totalCell(_fmt(_totalBalance), 100),
        ],
      ),
    );
  }
