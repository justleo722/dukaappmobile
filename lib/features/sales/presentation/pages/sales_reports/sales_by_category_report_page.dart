...
class _SalesByCategoryReportPageState extends ConsumerState<SalesByCategoryReportPage> {
  ...
  Future<void> _loadItems({String? from, String? to}) async {
    try {
      ...
      final data = await repo.fetchReportSalesByCategory(from: from, to: to);
      if (!mounted) return;
      setState(() {
  ...
  double get _totalAmount => _filteredItems.fold(0, (s, i) => s + i.total);
  ...
  Future<void> _exportPdf() async {
      ...
      pw.MultiPage(
        ...
        build: (context) => [
          pw.TableHelper.fromTextArray(
            ...
            headers: ['S/N', 'Category', 'Last Sale', 'Total', 'Profit'],
            data: [
              ...items.map((i) => [
                    '${i.sn}',
                    i.category,
              ...
              ['', 'TOTAL', '', _fmt(_totalAmount), _fmt(_totalProfit)],
            ],
          ),
        ],
    ...
    final file = File('${dir.path}/SalesByCategoryReport_$_exportDateTime.pdf');
  ...
  Future<void> _exportExcel() async {
    ...
    excel.rename(excel.getDefaultSheet()!, 'Sales by Category Report');

    final sheet = excel['Sales by Category Report'];

    final headers = ['S/N', 'Category', 'Last Sale', 'Total', 'Profit'];
    ...
    final dir = await getApplicationDocumentsDirectory();
    final file = File('${dir.path}/SalesByCategoryReport_$_exportDateTime.xlsx');
  ...
  Widget _buildTable(List<_CategoryData> items) {
    return Container(
      margin:
  ...
  Widget _buildTableRow(_CategoryData item, int index) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _nameCell(item.category, 120),
          _dataCell(item.lastSale, 100),
          _dataCell(_fmt(item.total), 100),
          _profitCell(item.profit, 100),
        ],
      ),
    );
  }
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _totalCell('', 100),
          _totalCell(_fmt(_totalAmount), 100),
          _totalProfitCell(_totalProfit, 100),
        ],
      ),
    );
  }
