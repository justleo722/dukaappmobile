...
class _CreditSalesReportPageState extends ConsumerState<CreditSalesReportPage> {
  ...
  Future<void> _loadItems({String? from, String? to}) async {
    try {
      ...
      final data = await repo.fetchReportCreditSales(from: from, to: to);
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _exportPdf() async {
      ...
      pw.MultiPage(
        ...
        build: (context) => [
          pw.TableHelper.fromTextArray(
            ...
            headers: ['S/N', 'Date', 'Customer', 'Type', 'Total', 'Paid', 'Balance'],
            data: [
              ...items.map((i) => [
                '${i.sn}', i.date, i.customer, i.type,
    ...
    final file = File('${dir.path}/CreditSalesReport_$_exportDateTime.pdf');
  ...
  Future<void> _exportExcel() async {
    ...
    excel.rename(excel.getDefaultSheet()!, 'Credit Sales Report');

    final sheet = excel['Credit Sales Report'];

    final headers = ['S/N', 'Date', 'Customer', 'Type', 'Total', 'Paid', 'Balance'];
    ...
    final dir = await getApplicationDocumentsDirectory();
    final file = File('${dir.path}/CreditSalesReport_$_exportDateTime.xlsx');
  ...
  Widget _buildTable(List<_CreditSaleItem> items) {
    final fmt = NumberFormat('#,###');
    return Container(
  ...
  Widget _buildTableRow(_CreditSaleItem item, int index) {
