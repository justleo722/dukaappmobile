...
class _SalesByStaffReportPageState extends ConsumerState<SalesByStaffReportPage> {
  ...
  Future<void> _loadItems({String? from, String? to}) async {
    try {
      ...
      final data = await repo.fetchReportSalesByStaff(from: from, to: to);
      if (!mounted) return;
      setState(() {
  ...
  List<_StaffSale> get _filteredItems {
    ...
    return _allItems.where((item) =>
        item.staff.toLowerCase().contains(query)).toList();
  }
  ...
  Future<void> _exportPdf() async {
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        header: (context) => pw.Column(
          ...
          children: [
            pw.Text('Sales by Staff Report', style: pw.TextStyle(font: pw.Font.helveticaBold(), fontSize: 16)),
        ...
        build: (context) => [
          pw.TableHelper.fromTextArray(
            ...
            headers: ['S/N', 'Staff', 'Total'],
            data: [
              ...items.map((i) => [
                '${i.sn}', i.staff, _fmt(i.total),
              ]),
    ...
    final file = File('${dir.path}/SalesByStaffReport_$_exportDateTime.pdf');
  ...
  Future<void> _exportExcel() async {
    ...
    excel.rename(excel.getDefaultSheet()!, 'Sales by Staff');

    final sheet = excel['Sales by Staff'];

    final headers = ['S/N', 'Staff', 'Total'];
    ...
    final dir = await getApplicationDocumentsDirectory();
    final file = File('${dir.path}/SalesByStaffReport_$_exportDateTime.xlsx');
  ...
  Widget _buildTable(List<_StaffSale> items) {
    return Container(
  ...
  Widget _buildTableRow(_StaffSale item, int index) {
