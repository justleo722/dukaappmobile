...
class _TotalSalesReportPageState extends ConsumerState<TotalSalesReportPage> {
  ...
  Future<void> _loadItems({String? from, String? to}) async {
    try {
      ...
      final data = await repo.fetchReportTotalSales(from: from, to: to);
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _exportPdf() async {
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        header: (context) => pw.Column(
          ...
          children: [
            pw.Text('Total Sales Report', style: pw.TextStyle(font: pw.Font.helveticaBold(), fontSize: 16)),
        ...
        build: (context) => [
          pw.TableHelper.fromTextArray(
            ...
            headers: ['S/N', 'Date', 'Type', 'Profit', 'Total', 'Paid', 'Balance'],
            data: [
              ...items.map((i) => [
                '${i.sn}', i.date, i.type, fmt.format(i.profit),
                fmt.format(i.total), fmt.format(i.paid), fmt.format(i.balance),
              ]),
              ['', 'TOTAL', '', fmt.format(_totalProfit), fmt.format(_totalTotal), fmt.format(_totalPaid), fmt.format(_totalBalance)],
            ],
          ),
        ],
    ...
    final file = File('${dir.path}/TotalSalesReport_$_exportDateTime.pdf');
  ...
  Future<void> _exportExcel() async {
    ...
    excel.rename(excel.getDefaultSheet()!, 'Total Sales Report');

    final sheet = excel['Total Sales Report'];

    final headers = ['S/N', 'Date', 'Type', 'Profit', 'Total', 'Paid', 'Balance'];
    ...
    final dir = await getApplicationDocumentsDirectory();
    final file = File('${dir.path}/TotalSalesReport_$_exportDateTime.xlsx');
  ...
  Widget _buildTable(List<_SaleItem> items) {
    final fmt = NumberFormat('#,###');
    return Container(
  ...
  Widget _buildTableRow(_SaleItem item, int index) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _dataCell(fmt.format(item.profit), 80),
          _dataCell(fmt.format(item.total), 80),
          _dataCell(fmt.format(item.paid), 80),
          _dataCell(fmt.format(item.balance), 80),
        ],
      ),
    );
  }
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _totalCell('', 70),
          _totalCell(fmt.format(_totalProfit), 80),
          _totalCell(fmt.format(_totalTotal), 80),
          _totalCell(fmt.format(_totalPaid), 80),
          _totalCell(fmt.format(_totalBalance), 80),
        ],
      ),
    );
  }
