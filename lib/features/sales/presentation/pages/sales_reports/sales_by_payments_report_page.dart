...
class _SalesByPaymentsReportPageState extends ConsumerState<SalesByPaymentsReportPage> {
  ...
  Future<void> _loadItems({String? from, String? to}) async {
    try {
      ...
      final data = await repo.fetchReportSalesByPayment(from: from, to: to);
      if (!mounted) return;
      setState(() {
  ...
  List<_PaymentData> get _filteredItems {
    ...
    return _allItems
        .where((item) => item.mode.toLowerCase().contains(query))
        .toList();
  }
  ...
  Future<void> _exportPdf() async {
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        header: (context) => pw.Column(
          ...
          children: [
            pw.Text('Sales by Payments Report',
        ...
        build: (context) => [
          pw.TableHelper.fromTextArray(
            ...
            headers: ['S/N', 'Payment Mode', 'Total'],
            data: [
              ...items.map((i) => [
                    '${i.sn}',
                    i.mode,
    ...
    final file = File('${dir.path}/SalesByPaymentsReport_$_exportDateTime.pdf');
  ...
  Future<void> _exportExcel() async {
    ...
    excel.rename(excel.getDefaultSheet()!, 'Sales by Payments Report');

    final sheet = excel['Sales by Payments Report'];

    final headers = ['S/N', 'Payment Mode', 'Total'];
    sheet.appendRow(headers.map((h) => xls.TextCellValue(h)).toList());

    for (final item in items) {
      sheet.appendRow([
        xls.DoubleCellValue(item.sn.toDouble()),
        xls.TextCellValue(item.mode),
    ...
    final dir = await getApplicationDocumentsDirectory();
    final file = File('${dir.path}/SalesByPaymentsReport_$_exportDateTime.xlsx');
  ...
  Widget _buildTable(List<_PaymentData> items) {
    return Container(
      margin:
  ...
  Widget _buildTableHeader() {
    return Container(
      ...
      child: Row(
        children: [
          _headerCell('S/N', 50),
          _headerCell('Payment Mode', 150),
          _headerCell('Total', 120),
        ],
      ),
    );
  }
  ...
  Widget _buildTableRow(_PaymentData item, int index) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _nameCell(item.mode, 150),
          _dataCell(_fmt(item.total), 120),
        ],
      ),
    );
  }
  ...
  Widget _buildTotalsRow() {
    return Container(
      ...
      child: Row(
        children: [
          _totalCell('', 50),
          _totalLabelCell('TOTAL', 150),
          _totalCell(_fmt(_totalAmount), 120),
        ],
      ),
    );
  }
