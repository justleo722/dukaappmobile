...
class _GenerateBarcodeReportPageState extends ConsumerState<GenerateBarcodeReportPage> {
  ...
  Future<void> _exportPdf() async {
    ...
    doc.addPage(pw.MultiPage(pageFormat: PdfPageFormat.a4.landscape, margin: const pw.EdgeInsets.all(20),
      header: (_) => pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
        pw.Text('Barcode Report', style: pw.TextStyle(font: pw.Font.helveticaBold(), fontSize: 16)),
      ...
      build: (_) => [pw.TableHelper.fromTextArray(
        ...
        headers: ['S/N', 'Product Name', 'Barcode', 'Selling Price', 'Available'],
        data: items.map((i) => ['${i.sn}', i.name, i.barcode, 'Tsh ${_fmt(i.sellingPrice)}', '${i.available}']).toList(),
      )],
    ));
    final dir = await getApplicationDocumentsDirectory();
    await File('${dir.path}/Barcodes_$_exportDateTime.pdf').writeAsBytes(await doc.save(), flush: true);
    await OpenFile.open('${dir.path}/Barcodes_$_exportDateTime.pdf');
  }
  ...
  Future<void> _exportExcel() async {
    ...
    excel.rename(excel.getDefaultSheet()!, 'Barcodes');
    final sheet = excel['Barcodes'];
    sheet.appendRow(['S/N', 'Product Name', 'Barcode', 'Selling Price', 'Available'].map((h) => xls.TextCellValue(h)).toList());
    for (final i in items) {
      sheet.appendRow([xls.DoubleCellValue(i.sn.toDouble()), xls.TextCellValue(i.name), xls.TextCellValue(i.barcode),
        xls.DoubleCellValue(i.sellingPrice), xls.DoubleCellValue(i.available.toDouble())]);
    }
    final bytes = excel.save(); if (bytes == null) return;
    final dir = await getApplicationDocumentsDirectory();
    final file = File('${dir.path}/Barcodes_$_exportDateTime.xlsx');
  ...
  PreferredSizeWidget _buildAppBar(BuildContext context) {
    return AppBar(backgroundColor: AppColors.card, elevation: 0,
      ...
      leadingWidth: 56, title: Text('Generate Barcodes', style: AppTypography.h6.copyWith(color: AppColors.textPrimary, fontWeight: FontWeight.w700)),
  ...
  Widget _buildTable(List<_BarcodeItem> items) {
  ...
  Widget _buildTableHeader() {
    return Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10), decoration: const BoxDecoration(color: AppColors.primary),
      child: Row(children: [
        _headerCell('S/N', 40), _headerCell('Product Name', 140), _headerCell('Barcode Image', 120),
        _headerCell('Barcode', 100), _headerCell('Selling Price', 80), _headerCell('Available', 70)]));
  }

  Widget _headerCell(String text, double width) {
    return SizedBox(width: width, child: Text(text, style: AppTypography.caption.copyWith(color: AppColors.textWhite, fontWeight: FontWeight.w700, fontSize: 10), textAlign: TextAlign.center));
  }

  Widget _buildTableRow(_BarcodeItem item, int index) {
    return Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10), decoration: BoxDecoration(color: index % 2 == 0 ? AppColors.card : AppColors.background),
      child: Row(children: [
        _dataCell('${item.sn}', 40), _nameCell(item.name, 140), _barcodeImageCell(item.barcode, 120),
        _barcodeTextCell(item.barcode, 100), _priceCell(item.sellingPrice, 80), _dataCell('${item.available}', 70)]));
  }

  Widget _dataCell(String t, double w) => SizedBox(width: w, child: Text(t, style: AppTypography.caption.copyWith(color: AppColors.textPrimary, fontSize: 10), textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis));
  Widget _nameCell(String t, double w) => SizedBox(width: w, child: Text(t, style: AppTypography.caption.copyWith(color: AppColors.textPrimary, fontWeight: FontWeight.w600, fontSize: 10), maxLines: 1, overflow: TextOverflow.ellipsis));
  Widget _barcodeTextCell(String t, double w) => SizedBox(width: w, child: Text(t, style: AppTypography.caption.copyWith(color: AppColors.textPrimary, fontSize: 9, fontFamily: 'monospace'), textAlign: TextAlign.center));

  Widget _barcodeImageCell(String barcode, double width) {
    return SizedBox(
      width: width,
      height: 40,
      child: CustomPaint(painter: _BarcodePainter(code: barcode)),
    );
  }

  Widget _priceCell(double value, double width) {
    return SizedBox(width: width, child: Text('Tsh ${_fmt(value)}', style: AppTypography.caption.copyWith(color: AppColors.primary, fontWeight: FontWeight.w600, fontSize: 10), textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis));
  }
  ...
  Widget _buildEmptyState() {
    return Center(child: Padding(padding: const EdgeInsets.symmetric(horizontal: AppConstants.paddingXXL, vertical: 60), child: Column(mainAxisSize: MainAxisSize.min, children: [
      Container(width: 100, height: 100, decoration: BoxDecoration(color: Color(0xFF8B5CF6).withValues(alpha: 0.1), shape: BoxShape.circle),
        child: Icon(Icons.qr_code_rounded, size: 48, color: Color(0xFF8B5CF6).withValues(alpha: 0.4))),
...
class _BarcodePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final random = code.codeUnits;
    final barWidth = size.width / (code.length * 2 + 2);
    double x = barWidth;

    for (int i = 0; i < code.length; i++) {
      final isWide = random[i] % 2 == 0;
      final w = isWide ? barWidth * 1.8 : barWidth;
      canvas.drawRect(Rect.fromLTWH(x, 2, w, size.height - 14), paint);
      x += w + barWidth * 0.5;
    }

    final textPainter = TextPainter(
      text: TextSpan(text: code, style: TextStyle(color: AppColors.textPrimary, fontSize: 7, fontFamily: 'monospace')),
      textDirection: ui.TextDirection.ltr,
    );
    textPainter.layout();
    textPainter.paint(canvas, Offset((size.width - textPainter.width) / 2, size.height - 10));
  }
