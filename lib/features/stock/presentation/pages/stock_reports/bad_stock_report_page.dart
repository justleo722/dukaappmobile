...
class _BadStockReportPageState extends ConsumerState<BadStockReportPage> {
  ...
  Future<void> _exportPdf() async {
    ...
    await File('${dir.path}/BadStock_$_exportDateTime.pdf').writeAsBytes(await doc.save(), flush: true);
    await OpenFile.open('${dir.path}/BadStock_$_exportDateTime.pdf');
  }
  ...
  Future<void> _exportExcel() async {
    ...
    excel.rename(excel.getDefaultSheet()!, 'Bad Stock');
    final sheet = excel['Bad Stock'];
    ...
    final dir = await getApplicationDocumentsDirectory();
    final file = File('${dir.path}/BadStock_$_exportDateTime.xlsx');
  ...
  PreferredSizeWidget _buildAppBar(BuildContext context) {
    return AppBar(backgroundColor: AppColors.card, elevation: 0,
      ...
      leadingWidth: 56, title: Text('Bad Stock', style: AppTypography.h6.copyWith(color: AppColors.textPrimary, fontWeight: FontWeight.w700)),
  ...
  Widget _buildTable(List<_BadStockItem> items) {
  ...
  Widget _buildTableRow(_BadStockItem item, int index) {
  ...
  Widget _buildEmptyState() {
    return Center(child: Padding(padding: const EdgeInsets.symmetric(horizontal: AppConstants.paddingXXL, vertical: 60), child: Column(mainAxisSize: MainAxisSize.min, children: [
      Container(width: 100, height: 100, decoration: BoxDecoration(color: AppColors.danger.withValues(alpha: 0.06), shape: BoxShape.circle),
        child: Icon(Icons.error_outline_rounded, size: 48, color: AppColors.danger.withValues(alpha: 0.4))),
