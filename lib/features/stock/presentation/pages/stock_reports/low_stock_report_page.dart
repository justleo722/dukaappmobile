...
class _LowStockReportPageState extends ConsumerState<LowStockReportPage> {
  ...
  List<_LowStockItem> get _filteredItems {
    ...
    return _allItems.where((i) => i.name.toLowerCase().contains(query)).toList();
  }
  ...
  Future<void> _exportPdf() async {
    ...
    doc.addPage(pw.MultiPage(
      ...
      header: (_) => pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
        pw.Text('Low Stock Report', style: pw.TextStyle(font: pw.Font.helveticaBold(), fontSize: 16)),
      ...
      build: (_) => [pw.TableHelper.fromTextArray(
        ...
        headers: ['S/N', 'Item Name', 'Quantity', 'Profit Estimate'],
        data: [
          ...items.map((i) => ['${i.sn}', i.name, '${i.quantity}', _fmt(i.profitEstimate)]),
          ['', 'TOTAL', '$_totalQty', _fmt(_totalProfit)],
        ],
      )],
    ));
    final dir = await getApplicationDocumentsDirectory();
    final file = File('${dir.path}/LowStock_$_exportDateTime.pdf');
  ...
  Future<void> _exportExcel() async {
    ...
    excel.rename(excel.getDefaultSheet()!, 'Low Stock');
    final sheet = excel['Low Stock'];
    sheet.appendRow(['S/N', 'Item Name', 'Quantity', 'Profit Estimate'].map((h) => xls.TextCellValue(h)).toList());
    for (final i in items) {
      sheet.appendRow([xls.DoubleCellValue(i.sn.toDouble()), xls.TextCellValue(i.name), xls.DoubleCellValue(i.quantity.toDouble()), xls.DoubleCellValue(i.profitEstimate)]);
    }
    sheet.appendRow([xls.TextCellValue(''), xls.TextCellValue('TOTAL'), xls.DoubleCellValue(_totalQty.toDouble()), xls.DoubleCellValue(_totalProfit)]);
    final bytes = excel.save();
    if (bytes == null) return;
    final dir = await getApplicationDocumentsDirectory();
    final file = File('${dir.path}/LowStock_$_exportDateTime.xlsx');
  ...
  PreferredSizeWidget _buildAppBar(BuildContext context) {
    return AppBar(
      ...
      leadingWidth: 56,
      title: Text('Low Stock', style: AppTypography.h6.copyWith(color: AppColors.textPrimary, fontWeight: FontWeight.w700)),
      centerTitle: true,
  ...
  Widget _buildShopHeader() {
    return Container(width: double.infinity, padding: const EdgeInsets.symmetric(horizontal: AppConstants.paddingLG, vertical: 12), color: AppColors.card,
      child: Row(children: [
        Icon(Icons.storefront_rounded, size: 18, color: AppColors.primary), const SizedBox(width: 8),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(_currentDateTime, style: AppTypography.caption.copyWith(color: AppColors.textSecondary, fontSize: 10)),
        ])),
      ]),
    );
  }
  ...
  Widget _buildTableHeader() {
    return Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10), decoration: const BoxDecoration(color: AppColors.primary),
      child: Row(children: [_headerCell('S/N', 50), _headerCell('Item Name', 180), _headerCell('Quantity', 80), _headerCell('Profit Estimate', 100)]),
    );
  }
  ...
  Widget _buildTableRow(_LowStockItem item, int index) {
    return Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10), decoration: BoxDecoration(color: index % 2 == 0 ? AppColors.card : AppColors.background),
      child: Row(children: [_dataCell('${item.sn}', 50), _nameCell(item.name, 180), _dataCell('${item.quantity}', 80), _profitCell(item.profitEstimate, 100)]),
    );
  }

  Widget _dataCell(String text, double width) {
    return SizedBox(width: width, child: Text(text, style: AppTypography.caption.copyWith(color: AppColors.textPrimary, fontSize: 11), textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis));
  }

  Widget _nameCell(String text, double width) {
    return SizedBox(width: width, child: Text(text, style: AppTypography.caption.copyWith(color: AppColors.textPrimary, fontWeight: FontWeight.w600, fontSize: 11), maxLines: 1, overflow: TextOverflow.ellipsis));
  }

  Widget _profitCell(double value, double width) {
    return SizedBox(width: width, child: Text(_fmt(value), style: AppTypography.caption.copyWith(color: AppColors.success, fontWeight: FontWeight.w600, fontSize: 11), textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis));
  }
  ...
  Widget _buildTotalsRow() {
    return Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10), decoration: BoxDecoration(color: AppColors.primary.withValues(alpha: 0.06),
      ...
      child: Row(children: [_totalW('', 50), _totalLabelW('TOTAL', 180), _totalW('$_totalQty', 80), _totalW(_fmt(_totalProfit), 100)]),
    );
  }

  Widget _totalW(String text, double width) {
    return SizedBox(width: width, child: Text(text, style: AppTypography.caption.copyWith(color: AppColors.primary, fontWeight: FontWeight.w700, fontSize: 11), textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis));
  }

  Widget _totalLabelW(String text, double width) {
    return SizedBox(width: width, child: Text(text, style: AppTypography.caption.copyWith(color: AppColors.primary, fontWeight: FontWeight.w800, fontSize: 11), maxLines: 1, overflow: TextOverflow.ellipsis));
  }
  ...
  Widget _buildEmptyState() {
    return Center(child: Padding(padding: const EdgeInsets.symmetric(horizontal: AppConstants.paddingXXL, vertical: 60), child: Column(mainAxisSize: MainAxisSize.min, children: [
      Container(width: 100, height: 100, decoration: BoxDecoration(color: AppColors.primary.withValues(alpha: 0.06), shape: BoxShape.circle),
        child: Icon(Icons.warning_amber_rounded, size: 48, color: AppColors.primary.withValues(alpha: 0.4))),
