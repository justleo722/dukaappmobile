...
class _LostStockReportPageState extends ConsumerState<LostStockReportPage> {
  ...
  double get _totalLoss => _filteredItems.fold(0, (s, i) => s + i.loss);
  ...
  Future<void> _exportPdf() async {
    ...
    doc.addPage(pw.MultiPage(pageFormat: PdfPageFormat.a4.landscape, margin: const pw.EdgeInsets.all(20),
      header: (_) => pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
        pw.Text('Lost Stock Report', style: pw.TextStyle(font: pw.Font.helveticaBold(), fontSize: 16)),
      ...
      build: (_) => [pw.TableHelper.fromTextArray(
        ...
        headers: ['S/N', 'Item Name', 'Quantity', 'Loss'],
        data: [...items.map((i) => ['${i.sn}', i.name, '${i.quantity}', _fmt(i.loss)]), ['', 'TOTAL', '$_totalQty', _fmt(_totalLoss)]])],
    ));
    final dir = await getApplicationDocumentsDirectory();
    await File('${dir.path}/LostStock_$_exportDateTime.pdf').writeAsBytes(await doc.save(), flush: true);
    await OpenFile.open('${dir.path}/LostStock_$_exportDateTime.pdf');
  }
  ...
  Future<void> _exportExcel() async {
    ...
    excel.rename(excel.getDefaultSheet()!, 'Lost Stock');
    final sheet = excel['Lost Stock'];
    sheet.appendRow(['S/N', 'Item Name', 'Quantity', 'Loss'].map((h) => xls.TextCellValue(h)).toList());
    for (final i in items) { sheet.appendRow([xls.DoubleCellValue(i.sn.toDouble()), xls.TextCellValue(i.name), xls.DoubleCellValue(i.quantity.toDouble()), xls.DoubleCellValue(i.loss)]); }
    sheet.appendRow([xls.TextCellValue(''), xls.TextCellValue('TOTAL'), xls.DoubleCellValue(_totalQty.toDouble()), xls.DoubleCellValue(_totalLoss)]);
    final bytes = excel.save(); if (bytes == null) return;
    final dir = await getApplicationDocumentsDirectory();
    final file = File('${dir.path}/LostStock_$_exportDateTime.xlsx');
  ...
  PreferredSizeWidget _buildAppBar(BuildContext context) {
    return AppBar(backgroundColor: AppColors.card, elevation: 0,
      ...
      leadingWidth: 56, title: Text('Lost Stock', style: AppTypography.h6.copyWith(color: AppColors.textPrimary, fontWeight: FontWeight.w700)),
  ...
  Widget _buildTable(List<_LostStockItem> items) {
  ...
  Widget _buildTableHeader() {
    return Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10), decoration: const BoxDecoration(color: AppColors.primary),
      child: Row(children: [_headerCell('S/N', 50), _headerCell('Item Name', 180), _headerCell('Quantity', 80), _headerCell('Loss', 100)]));
  }
  ...
  Widget _buildTableRow(_LostStockItem item, int index) {
    return Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10), decoration: BoxDecoration(color: index % 2 == 0 ? AppColors.card : AppColors.background),
      child: Row(children: [_dataCell('${item.sn}', 50), _nameCell(item.name, 180), _dataCell('${item.quantity}', 80), _lossCell(item.loss, 100)]));
  }
  ...
  Widget _lossCell(double v, double w) => SizedBox(width: w, child: Text(_fmt(v), style: AppTypography.caption.copyWith(color: AppColors.danger, fontWeight: FontWeight.w600, fontSize: 11), textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis));
  ...
  Widget _buildTotalsRow() {
    return Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10), decoration: BoxDecoration(color: AppColors.primary.withValues(alpha: 0.06),
      ...
      child: Row(children: [_totalW('', 50), _totalLabelW('TOTAL', 180), _totalW('$_totalQty', 80), _totalW(_fmt(_totalLoss), 100)]));
  }
  ...
  Widget _buildEmptyState() {
    return Center(child: Padding(padding: const EdgeInsets.symmetric(horizontal: AppConstants.paddingXXL, vertical: 60), child: Column(mainAxisSize: MainAxisSize.min, children: [
      Container(width: 100, height: 100, decoration: BoxDecoration(color: AppColors.textSecondary.withValues(alpha: 0.1), shape: BoxShape.circle),
        child: Icon(Icons.remove_shopping_cart_rounded, size: 48, color: AppColors.textSecondary.withValues(alpha: 0.4))),
