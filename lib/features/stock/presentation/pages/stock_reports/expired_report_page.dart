...
class _ExpiredReportPageState extends ConsumerState<ExpiredReportPage> {
  ...
  Future<void> _exportPdf() async {
    ...
    doc.addPage(pw.MultiPage(pageFormat: PdfPageFormat.a4.landscape, margin: const pw.EdgeInsets.all(20),
      header: (_) => pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
        pw.Text('Expired Products Report', style: pw.TextStyle(font: pw.Font.helveticaBold(), fontSize: 16)),
    ...
    await File('${dir.path}/Expired_$_exportDateTime.pdf').writeAsBytes(await doc.save(), flush: true);
    await OpenFile.open('${dir.path}/Expired_$_exportDateTime.pdf');
  }
  ...
  Future<void> _exportExcel() async {
    ...
    excel.rename(excel.getDefaultSheet()!, 'Expired');
    final sheet = excel['Expired'];
    ...
    final dir = await getApplicationDocumentsDirectory();
    final file = File('${dir.path}/Expired_$_exportDateTime.xlsx');
  ...
  PreferredSizeWidget _buildAppBar(BuildContext context) {
    return AppBar(backgroundColor: AppColors.card, elevation: 0,
      ...
      leadingWidth: 56, title: Text('Expired', style: AppTypography.h6.copyWith(color: AppColors.textPrimary, fontWeight: FontWeight.w700)),
  ...
  Widget _buildTable(List<_ExpiredItem> items) {
  ...
  Widget _buildTableRow(_ExpiredItem item, int index) {
  ...
  Widget _buildEmptyState() {
    return Center(child: Padding(padding: const EdgeInsets.symmetric(horizontal: AppConstants.paddingXXL, vertical: 60), child: Column(mainAxisSize: MainAxisSize.min, children: [
      Container(width: 100, height: 100, decoration: BoxDecoration(color: AppColors.danger.withValues(alpha: 0.06), shape: BoxShape.circle),
        child: Icon(Icons.event_busy_rounded, size: 48, color: AppColors.danger.withValues(alpha: 0.4))),
