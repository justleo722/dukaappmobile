...
class EditRecipePage extends ConsumerStatefulWidget {
  ...
  const EditRecipePage({super.key, required this.recipeData});
...
class _EditRecipePageState extends ConsumerState<EditRecipePage> {
  ...
  void initState() {
    ...
    _nameController = TextEditingController(text: data['name'] ?? data['recipe_name'] ?? '');
    _yieldController = TextEditingController(text: (data['yieldAmount'] ?? data['yield_quantity'] ?? '1').toString());

    // Pre-populate from API ingredients array (preferred) or legacy string list
    final apiIngredients = data['ingredients'];
    if (apiIngredients is List && apiIngredients.isNotEmpty && apiIngredients.first is Map) {
      for (final m in apiIngredients) {
        final map = m as Map<String, dynamic>;
        _ingredients.add(_Ingredient(
          materialId: (map['raw_material_id'] ?? map['material_id'] ?? '').toString(),
          materialName: (map['material_name'] ?? map['name'] ?? '').toString(),
          quantity: (map['quantity_required'] ?? map['quantity'] ?? '1').toString(),
          unit: (map['unit'] ?? 'kg').toString(),
        ));
      }
    } else if (apiIngredients is List) {
      for (final name in apiIngredients) {
        _ingredients.add(_Ingredient(materialName: name.toString()));
      }
    }
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (status == 'success') {
        ...
        final msg = result['message']?.toString() ?? 'Failed to update recipe';
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
  ...
  void _addIngredient() => setState(() => _ingredients.add(_Ingredient()));
            ...
            Container(
              ...
              child: _isLoadingMaterials
                  ...
                  : DropdownButton<String>(
                      ...
                      hint: Text(
                        ing.materialName.isNotEmpty ? ing.materialName : 'Select',
                        style: AppTypography.caption.copyWith(color: AppColors.textHint, fontSize: 10),
      ...
      child: Row(children: [
        ...
        Expanded(flex: 2, child: SizedBox(height: AppConstants.buttonHeight, child: ElevatedButton(
          ...
          child: _isSaving
              ...
              : Text('Update Recipe', style: AppTypography.buttonLarge),
        ))),
      ]),
    );
  }
