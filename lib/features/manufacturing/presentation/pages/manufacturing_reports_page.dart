...
class ManufacturingReportsPage extends StatelessWidget {
  const ManufacturingReportsPage({super.key});

  static const List<_ReportData> _reports = [
    _ReportData(
      name: 'Daily Production Summary',
      description: 'View daily production output and status.',
      icon: Icons.factory_rounded,
      color: AppColors.primary,
      route: '/manufacturing/reports/daily-production',
    ),
    _ReportData(
      name: 'Raw Material Consumption',
      description: 'Track raw material usage per production.',
      icon: Icons.inventory_2_rounded,
      color: AppColors.secondary,
      route: '/manufacturing/reports/raw-material-consumption',
    ),
    _ReportData(
      ...
      route: '/manufacturing/reports/production-cost-analysis',
    ),
    _ReportData(
      name: 'Low Stock Alert Report',
      description: 'Raw materials running low on stock.',
      icon: Icons.warning_amber_rounded,
      color: AppColors.warning,
      route: '/manufacturing/reports/low-stock-alert',
    ),
    _ReportData(
      name: 'Product by Recipe',
      description: 'Products grouped by their recipes.',
      icon: Icons.receipt_long_rounded,
      color: const Color(0xFF8B5CF6),
      route: '/manufacturing/reports/product-by-recipe',
    ),
  ];
    ...
    return Scaffold(
      ...
      body: SafeArea(
        ...
        child: ListView.builder(
          padding: EdgeInsets.only(top: 12, bottom: 24 + bottomPadding),
              ...
              onTap: () {
                if (report.route != null) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text('${report.name} coming soon',
  ...
  PreferredSizeWidget _buildAppBar(BuildContext context) {
    return AppBar(
      ...
      title: Column(children: [
        Text('Manufacturing Reports', style: AppTypography.h6.copyWith(color: AppColors.textPrimary, fontWeight: FontWeight.w700)),
        const SizedBox(height: 2),
        Text('View and export manufacturing reports.', style: AppTypography.caption.copyWith(color: AppColors.textSecondary, fontSize: 10)),
      ]),
      centerTitle: true,
      actions: [
        Padding(padding: const EdgeInsets.only(right: 12), child: Container(
          ...
          child: IconButton(onPressed: null, icon: Icon(Icons.download_rounded, color: AppColors.textHint, size: 20)),
        )),
      ],
